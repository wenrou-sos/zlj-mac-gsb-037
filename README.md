...
4. **📜 生产记录**
   - 全量查询所有上报记录（显示当前生效值，可追溯原始值）
   - 按工单号、上报人筛选分页
   - 查看任意记录的修订时间线

5. **✅ 纠错审批**
   - 按状态筛选（待审批 / 已批准 / 已驳回）纠错申请
   - 查看"当前生效值 → 修正值"差异对比与纠错原因
   - 批准：生成不可变修订版本，事务内按全部最新有效记录重算工单完成数/不良数/工时/状态/告警
   - 驳回：必须填写审批意见，申请可重新提交
...
2. **✏️ 生产上报**
   ...
   - 数量超限二次确认
   - 自动累计更新工单统计
3. **📜 生产记录** - 仅查看本人上报记录，可对本人记录发起纠错申请
4. **📝 我的纠错**
   - 对本人记录提交纠错申请（修正完成数/不良数/工时/不良原因 + 必填纠错原因）
   - 原记录不可直接修改，需主管批准后生效
   - 同一记录同时最多一条待审批申请
   - 查看申请审批进度与记录修订时间线
...
| `production_records` | 生产记录表（流水，含当前生效修订指针） |
| `record_correction_requests` | 纠错申请表（生成列唯一索引保证同一记录最多一条待审批） |
| `record_revisions` | 修订版本表（只增不改，触发器禁止 UPDATE） |
...
| GET | `/api/records` | 生产记录（分页，含生效值与修订状态） |
| POST | `/api/records` | 上报生产进度（事务更新工单） |
| GET | `/api/records/:id/revisions` | 记录修订时间线（原始记录+全部修订版本） |
| POST | `/api/corrections` | 提交纠错申请（仅操作工、仅本人记录） |
| GET | `/api/corrections` | 纠错申请列表（状态筛选；操作工仅见本人） |
| GET | `/api/corrections/:id` | 纠错申请详情（含差异快照） |
| POST | `/api/corrections/:id/approve` | 主管批准（事务：修订版本+工单重算） |
| POST | `/api/corrections/:id/reject` | 主管驳回（须填审批意见） |
...
```
1. 操作工 → 生产记录 → 对本人记录点"申请纠错" → 填写修正值与纠错原因
   （原记录不会被修改；同一记录同时最多一条待审批申请）
2. 主管 → 纠错审批 → 查看"当前生效值 → 修正值"差异与原因
3a. 主管批准 → 事务内依次锁定申请/记录/工单：
    新增不可变修订版本 → 更新记录生效指针 → 标记申请已批准
    → 按全部最新有效记录重算工单完成数/不良数/工时/状态/不良率告警
    → 任一步失败整体回滚
3b. 主管驳回 → 填写审批意见，申请关闭，数据不变，操作工可重新申请
4. 全程审计：原始记录永不修改，每次批准生成递增修订版本，
   记录详情与"修订时间线"可完整追溯（谁申请、为何修、谁批准、何时生效）
```

## 数据库迁移（已有环境升级）

全新安装执行 `npm run init:db` 即可（init.sql 已包含全部表结构）。
已有数据库执行纠错审批迁移脚本（兼容 MySQL 5.7+，幂等可重复执行）：
...
```bash
# 后端集成测试（需先启动服务于 3002 端口；纠错测试会自动应用迁移）
cd server
PORT=3002 npm start &      # 另开服务
npm test                   # 既有 API 测试 + 纠错审批测试（权限/并发/回滚/重算）
